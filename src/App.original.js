...
const sanitizeAndParse = (content) => {
  // Extract citations and format them properly
  let processedContent = content;
  const citations = [];
  let citationIndex = 1;

  // Pattern to match markdown links with (domain) format
  const citationPattern = /\(?\[([^\]]*)\]\((https?:\/\/[^\)]+)\)\)?/g;

  processedContent = processedContent.replace(citationPattern, (match, text, url) => {
    // If it's a citation-style link (domain name in brackets)
    ...
    // Otherwise, keep as normal link
    return `<a href="${url}" target="_blank" rel="noopener">${text}</a>`;
  });

  // Also handle bare URLs in parentheses (windowsforum.com) format
  ...
  // Add citation list at the end if there are citations
...
async function sendChatMessage(messageContent, onChunkReceived, signal, captchaToken = null) {
  try {
            ...
            try {
              ...
              switch (parsedData.type) {
                ...
                case 'response.content_part.done':
                  // Final annotations might come here
    ...
    return { text: partialText, annotations }; // Return text and annotations
  } catch (error) {
...
const ChatWindow = ({ userAvatar, userName, userId }) => {
  // Check if user is a guest: userId is either null, or a string starting with 'guest_'
    ...
    try {
      const result = await sendChatMessage(
        ...
        (partialText, annotations) => {
          // Format text with citations if available
          ...
          setStreamingMessage({ role: 'ai', content: formattedText, annotations }); // Update streaming message
        },
      ...
      if (result && result.text) {
        ...
        // Only add file-based annotations if present (not web citations which are handled in sanitizeAndParse)
        ...
        setMessages((prev) => [
          ...prev,
          { role: 'ai', content: sanitizeAndParse(finalContent) },
        ]);
        setStreamingMessage(null);
